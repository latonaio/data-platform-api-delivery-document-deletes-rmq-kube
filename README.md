# data-platform-api-delivery-document-deletes-rmq-kube

data-platform-api-delivery-document-deletes-rmq-kube は、周辺業務システム　を データ連携基盤 と統合することを目的に、API で入出荷伝票データに削除フラグを設定するマイクロサービスです。  
https://xxx.xxx.io/api/API_DELIVERY_DOCUMENT_SRV/deletes/

## 動作環境

data-platform-api-delivery-document-deletes-rmq-kube の動作環境は、次の通りです。  
・ OS: LinuxOS （必須）  
・ CPU: ARM/AMD/Intel（いずれか必須）  


## 本レポジトリ が 対応する API サービス
data-platform-api-delivery-document-deletes-rmq-kube が対応する APIサービス は、次のものです。

APIサービス URL: https://xxx.xxx.io/api/API_DELIVERY_DOCUMENT_SRV/deletes/

## 本レポジトリ に 含まれる API名
data-platform-api-delivery-document-deletes-rmq-kube には、次の API をコールするためのリソースが含まれています。  

* A_Header（入出荷伝票 - ヘッダデータ）
* A_Item（入出荷伝票 - 明細データ）

## API への 値入力条件 の 初期値
data-platform-api-delivery-document-deletes-rmq-kube において、API への値入力条件の初期値は、入力ファイルレイアウトの種別毎に、次の通りとなっています。  
...
```
	"api_schema": "DPFMDeliveryDocumentDeletes",
	"accepter": ["Header"],
```
...
以下の sample.json の例は 入出荷伝票 の ヘッダデータ がに削除フラグが設定された結果の JSON の例です。  
