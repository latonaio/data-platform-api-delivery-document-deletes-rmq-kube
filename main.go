...
import (
	dpfm_api_caller "data-platform-api-delivery-document-deletes-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncDeliveryDocumentDeletes(accepter, &input, &output, l)
	if len(errs) != 0 {
